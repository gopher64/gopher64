...
impl Device {
    pub fn new() -> Device {
        ...
        Device {
            ...
            cpu: cpu::Cpu {
                cop0: cop0::Cop0 {
                    regs: vec![0; cop0::COP0_REGS_COUNT as usize],
                    reg_write_masks: vec![0; cop0::COP0_REGS_COUNT as usize],
                    reg_latch: 0,
                    is_event: false,
                    instrs: vec![cop0::reserved; 32],
                    instrs2: vec![cop0::reserved; 32],
                    tlb_lut_w: vec![
                        tlb::TlbLut {
                            address: 0,
                            cached: false,
                ...
                cop1: cop1::Cop1 {
                    ...
                    fgr32: vec![[0; 4]; 32],
                    fgr64: vec![[0; 8]; 32],
                    instrs: vec![cop1::reserved; 32],
                    b_instrs: vec![cop1::reserved; 4],
                    s_instrs: vec![cop1::reserved; 64],
                    d_instrs: vec![cop1::reserved; 64],
                    w_instrs: vec![cop1::reserved; 64],
                    l_instrs: vec![cop1::reserved; 64],
                },
                cop2: cop2::Cop2 {
                    instrs: vec![cop2::reserved; 32],
                    reg_latch: 0,
                },
                ...
                hi: 0,
                running: false,
                instrs: vec![cop0::reserved; 64],
                special_instrs: vec![cop0::reserved; 64],
                regimm_instrs: vec![cop0::reserved; 32],
                events: vec![
                    events::Event {
                        enabled: false,
                        count: u64::MAX,
            ...
            cart: cart::Cart {
                ...
                sc64: cart::sc64::Sc64 {
                    regs: vec![0; cart::sc64::SC64_REGS_COUNT as usize],
                    regs_locked: true,
                    cfg: vec![0; cart::sc64::SC64_CFG_COUNT as usize],
                    sector: 0,
                    buffer: vec![0; 8192],
            ...
            memory: memory::Memory {
                fast_read: vec![unmapped::read_mem_fast; 0x2000],
                memory_map_read: vec![unmapped::read_mem; 0x2000],
                memory_map_write: vec![unmapped::write_mem; 0x2000],
                icache: vec![
                    cache::ICache {
                        valid: false,
                        index: 0,
                        tag: 0,
                        words: vec![0; 8],
                        instruction: vec![cop0::reserved; 8],
                    };
                    512
                ],
                dcache: vec![
            ...
            rdram: rdram::Rdram {
                mem: vec![],
                size: 0x800000,
                regs: vec![vec![0; rdram::RDRAM_REGS_COUNT as usize]; 4],
            },
            rsp: rsp_interface::Rsp {
                cpu: rsp_cpu::Cpu {
                    ...
                    special_instrs: vec![rsp_su_instructions::reserved; 64],
                    regimm_instrs: vec![rsp_su_instructions::reserved; 32],
                    cop0_instrs: vec![rsp_su_instructions::reserved; 32],
                    cop2_instrs: vec![rsp_su_instructions::reserved; 32],
                    lwc2_instrs: vec![rsp_su_instructions::reserved; 32],
                    swc2_instrs: vec![rsp_su_instructions::reserved; 32],
                    instrs: vec![rsp_su_instructions::reserved; 64],
                    vec_instrs: vec![rsp_vu_instructions::reserved; 64],
                },
                regs: vec![0; rsp_interface::SP_REGS_COUNT as usize],
                regs2: vec![0; rsp_interface::SP_REGS2_COUNT as usize],
                mem: vec![0; 0x2000],
                last_status_value: 0,
                run_after_dma: false,
            ...
            rdp: rdp::Rdp {
                regs_dpc: vec![0; rdp::DPC_REGS_COUNT as usize],
                regs_dps: vec![0; rdp::DPS_REGS_COUNT as usize],
                wait_frozen: false,
                last_status_value: 0,
            },
            mi: mi::Mi {
                regs: vec![0; mi::MI_REGS_COUNT as usize],
            },
            pi: pi::Pi {
                regs: vec![0; pi::PI_REGS_COUNT as usize],
            },
            ai: ai::Ai {
                regs: vec![0; ai::AI_REGS_COUNT as usize],
                last_read: 0,
                freq: 33600,
                delayed_carry: false,
                fifo: [ai::AiDma {
            ...
            si: si::Si {
                regs: vec![0; si::SI_REGS_COUNT as usize],
                dma_dir: si::DmaDir::None,
            },
            ri: ri::Ri {
                regs: vec![0; ri::RI_REGS_COUNT as usize],
                ram_init: false,
            },
            vi: vi::Vi {
                regs: vec![0; vi::VI_REGS_COUNT as usize],
                clock: 0,
                field: 0,
                delay: 0,
